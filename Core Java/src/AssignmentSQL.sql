...
mysql> SELECT * FROM orders WHERE amt>(SELECT AVG(AMT) FROM orders o LEFT JOIN customers c ON o.CNUM=c.CNUM);
+------+---------+------------+------+------+
| ONUM | AMT     | ODATE      | CNUM | SNUM |
+------+---------+------------+------+------+
| 3005 | 5160.45 | 1990-10-03 | 2003 | 1002 |
...
mysql> SELECT ONUM,s.SNUM,s.SCITY As salespeople s,c.CITY AS c.CITY FROM orders o JOIN salespeolpe s ON s.SNUM=o.SNUM JOIN customers c ON o.CNUM =c.CNUM WHERE s.CITY <> c.CITY;
